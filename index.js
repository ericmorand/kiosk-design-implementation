...
const config = merge.recursive(true, require('./config/common'), require('./config/develop'));
    ...
    return super.start(config).then(
      function (components) {
        ...
        components.forEach(function (component) {
          output += '<li><a href="' + path.join(component.name) + '">' + component.name + '</a></li>';
        });
      ...
      function () {
        return pluginRenderComponent.call(that, plugin, component).then(
          function (component) {
            ...
            return write(renderResult, path.join(wwwPath, component.name)).then(
              function (files) {
                // watch dependencies
                let watcher = null;
